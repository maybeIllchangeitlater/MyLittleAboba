...
namespace s21{
    ...
    std::vector<std::pair<vec, vec>> DataLoader::CreateSample(size_t batch_size, size_t start_from, Mode mode, bool shuffle){

        auto & data = mode == kTrain ? data_ : test_data_;
        std::vector<std::pair<vec, vec>> sample;
...
//       Mx ideal(1, out_);
       ideal[--index] = 1.0;
       data.emplace_back(ideal, vec(in_));

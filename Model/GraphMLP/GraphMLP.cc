#include "GraphMLP.h"
namespace s21 {
GraphMLP::GraphMLP(std::vector<size_t> topology, s21::DataLoader *dl,
...
std::vector<size_t> GraphMLP::Topology() const noexcept {
  ...
  for (const auto &v : layers_) res.emplace_back(v.Size());
  return res;
}
...
double GraphMLP::GetError(const std::vector<double> &ideal) const {
  ...
  return std::sqrt(sum / layers_.back().Size());
}
...
void GraphMLP::Out(std::ostream &out) const {
  ...
  for (const auto &l : layers_) out << l.Size() << " ";

  for (size_t i = 1; i < layers_.size(); ++i) out << layers_[i];
}
...
void GraphMLP::In(std::istream &in) {
  ...
  layers_.emplace_back(topology[0]);
  for (size_t i = 0; i < topology.size() - 1; ++i) {
    layers_.emplace_back(topology[i], topology[i + 1]);
    in >> layers_[i + 1];
  }
}

...
MatrixMLP::MatrixMLP(std::vector<size_t> topology, DataLoader *dl,
                     const char *activation_function_name)
  ...
  for (size_t i = 0; i < topology.size() - 1; ++i) {
    layers_.emplace_back(
        Matrix(topology[i], topology[i + 1], gen_, 0.0,
               (2.0 / std::sqrt(topology[i] * topology[i + 1]))),
...
void MatrixMLP::FeedForward(const std::vector<double> &in) {
  ...
  for (size_t i = 0; i < layers_.size() - 1; ++i) {
    layers_[i + 1].outputs_ =
        layers_[i].activated_outputs_ * layers_[i].weights_;
    layers_[i + 1].outputs_ += layers_[i].biases_;
    layers_[i + 1].activated_outputs_ =
        i < layers_.size() - 2
...
void MatrixMLP::BackPropogation(const std::vector<double> &ideal) {
  layers_[layers_.size() - 2].error_ =
      layers_.back().activated_outputs_ - ideal;
  // dZ = a - Y
  for (std::ptrdiff_t i = layers_.size() - 3; i >= 0; --i) {
    layers_[i].error_ =
        layers_[i + 1].error_.MulByTransposed(layers_[i + 1].weights_);
    Matrix der = layers_[i + 1].outputs_.Transform(activation_derivative_);
    layers_[i].error_ &= der;
    // dZi = dZi+1 * W.T hadamard product with ActDeriv(Zi) python - dZi =
    // dZi+1.dot(W.T) * ActDeriv(Zi)
  }
...
void MatrixMLP::UpdateWeights() {
  for (size_t i = 0; i < layers_.size() - 1; ++i) {
    Matrix weight_gradients =
        layers_[i].activated_outputs_.MulSelfTranspose(layers_[i].error_);
    // dW = a.T * dZ
...
std::vector<size_t> MatrixMLP::Topology() const noexcept {
  ...
  res.emplace_back(dl_->Inputs());
  for (size_t i = 0; i < layers_.size() - 1; ++i) {
    res.emplace_back(layers_[i].biases_.Size());
  }
  return res;
}
...
double MatrixMLP::GetError(const std::vector<double> &ideal) const {
  return std::sqrt(
      (layers_.back().activated_outputs_ - ideal).Pow2().Sum() /

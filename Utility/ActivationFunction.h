...
class ActivationFunction {
 public:
  ...
  /// Pros: outputs values between 0 and 1, making it suitable for problems that
  /// require probability-like outputs.\n Cons: can suffer from vanishing
  /// gradient slowing down training (bigger network the worse it is),\n
  ...
  /// Pros: efficient, wide range leading to faster convergence, solves
  /// vanishing gradient problem\n Cons: "dead neurons" which can get stack in
  /// always outputting 0s, not differentiable at 0
  static double ReLU(const double x) { return std::max(0.0, x); }

  static double ReLUDeriv(const double x) { return x > 0; }

  /// Pros: efficient, wide range leading to faster convergence, solves
  /// vanishing gradient problem, addresses dead neurons problem\n Cons:
  /// performance can be problem-dependent, The "leakiness" parameter needs to
  /// be tuned, choosing the right value can be challenging.
  ...
  /// Pros: ELU can help with faster convergence and mitigate some of the issues
  ...
  /// Pros: Tanh is zero-centered, which can help with faster convergence in
  /// some cases\n Cons: can suffer from vanishing gradient slowing down
  /// training (bigger network the worse it is), but not as bad as Sigmoid\n
  /// saturation but again, not as bad as Sigmoid
  static double Tanh(const double x) { return std::tanh(x); }

  static double TanhDeriv(const double x) { return 1.0 - tanh(x * x); }
  ...
  static const std::unordered_map<
      std::string, std::pair<double (*)(double), double (*)(double)>>

...
class Matrix {
 public:
   ...
   */
  Matrix(const size_t rows, const size_t cols, std::mt19937 &generator,
  ...
  /**
   * @brief Constructs vector-like matrix(1 by x where x is iterable object
   * size)\n Object must be iterable, have .size() method\n and have iterator
   * dereference value implicitly convertable to double\n
   */
  template <typename Iterable>
  Matrix(const Iterable &other) : Matrix(1, other.size()) {
  ...
  Matrix operator&(const Iterable &other) const {
    Matrix res(rows_, cols_);
    std::transform(begin(), end(), other.begin(), res.begin(),
                   std::multiplies<>());
    return res;
  }
  template <typename Iterable>
  ...
  Matrix operator*(const double num) const {
    Matrix res(rows_, cols_);
    auto it = res.begin();
    for (const auto &v : *this) *it++ = v * num;
    return res;
  }
  ...
  friend Matrix operator/(const double num, const Matrix &other) {
    ...
    auto it = res.begin();
    for (const auto &v : other) *it++ = num / v;
    return res;
  }
  Matrix &operator/=(const double num) noexcept {
    for (auto &v : *this) v /= num;
  ...
  Matrix Pow2() const {
    return Transform([](double x) { return x * x; });
  }
  ...
  double &operator()(const size_t i, const size_t j) noexcept {
    return matrix_[i][j];
  }
  const double &operator()(const size_t i, const size_t j) const noexcept {
    return matrix_[i][j];
  }
